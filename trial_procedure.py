...
class trial_procedure_cl:
    def __init__(self, params, paramsS, mode, print_outs=False):
        ...
        if "seed" in list(paramsS.keys()):
            self.rng = np.random.default_rng(paramsS["seed"])
        else:
            self.rng = np.random.default_rng(None)
        self.rand = {}
        self.rand["cor_go"] = self.rng.standard_normal()
        self.rand["cor_pause"] = self.rng.standard_normal()
        self.rand["cor_stop"] = self.rng.standard_normal()
    ...
    class event_cl:
        ...
        def run(self):
            ...
            if self.check_requirements():
                ### run the event
                if self.trial_procedure.print_outs:
            ...
            for sub_idx, sub_string in enumerate(string):
                ...
                if len(sub_string) == 1:
                    if sub_idx < len(string) - 1:
                        final_string.append(
                            self.get_condition_part(sub_string[0]) + " and "
                        )
                    else:
                    ...
                    for sub_sub_idx, sub_sub_string in enumerate(sub_string):
                        if sub_sub_idx < len(sub_string) - 1:
                            final_string.append(
                                self.get_condition_part(sub_sub_string) + " or "
                            )
                        elif sub_idx < len(string) - 1:
                            final_string.append(
                                self.get_condition_part(sub_sub_string) + " and "
                            )
                        else:
