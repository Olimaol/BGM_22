...
nx = 20  # along caudate-putamen axis (x)
b = 20  # along y and z axes
density = 80500  # neurons per mm^3
...
# =====================================
# Build periodic KDTree by tiling the cube in all 27 shifts
L = np.array([nx * d, b * d, b * d])
shifts = np.array(
...
for j in range(n_total):
    ...
    for i in neighbor_idxs:
        ...
        # half-gaussian connection probability
...
for pre_type in cell_types:
    ...
    for post_type in cell_types:
        ...
        dim_y_mm = b * d
        dim_z_mm = b * d
        # print(f"Central margin (mm): {margin_mm:.2f}")
        # print(f"Dimensions (mm): X={dim_x_mm:.2f}, Y={dim_y_mm:.2f}, Z={dim_z_mm:.2f}")
        # print(f"X max: {X.max():.2f}, Y max: {Y.max():.2f}, Z max: {Z.max():.2f}")
        # break
