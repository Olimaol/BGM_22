...
from BGM_22.neuronmodels import poisson_neuron_up_down, poisson_neuron, izhikevich2007_standard, izhikevich2007_fsi, izhikevich2003, integrator_neuron, izhikevich2003_modified, izhikevich2007_standard_new_noise
...
def set_params(params):
    ...
    for key, val in params.items():
        ...
        if len(key_split)>=2:
            ...
            if param_name.split('_')[-1]=='noise':# for noise params separate fucntion
                continue
...
def set_noise_values_new(params):
    """
    ...
    for key, val in params.items():
        ...
        if len(key_split)>=2:
            ...
            ### if pop is in network --> set param
            if pop_name in population_name_list and param_name.split('_')[-1]=='noise':
                if param_name=='rate_noise':
                    prob_noise_event=(val/1000)*dt()
                    setattr(get_population(pop_name), 'prob_noise_event', prob_noise_event)
                elif param_name=='rate_sd_noise':
                    rate = params[pop_name+'__rate_noise']
                    rate_dist=np.clip(np.random.normal(rate, val, get_population(pop_name).size),1./dt(),None)
                    isi_dist = 1/rate_dist
                    ### only want the variation of the isis, wihtout mean rate
                    isi_variations = isi_dist-(1./rate)
                    setattr(get_population(pop_name), 'isi_variations', isi_variations)
                else:
                    setattr(get_population(pop_name), param_name, val)
...
def BGM_new_noise(do_compile=False, compile_folder_name='annarchy_BGM_new_noise'):
    """
        generates a basal ganglia model (model_id = BGM_new_noise) and optionally compiles the network
        
        returns a list of the names of the populations an projections (for later access)
    """
    
    params = get_params('BGM_new_noise')
    ...
    cor_stop  = Population(params['cor_stop__size'],  poisson_neuron_up_down, name="cor_stop")
    ### Str Populations
    str_d1  = Population(params['str_d1__size'],  izhikevich2007_standard_new_noise, name="str_d1")
    str_d2  = Population(params['str_d2__size'],  izhikevich2007_standard, name="str_d2")
    str_fsi = Population(params['str_fsi__size'], izhikevich2007_fsi, name="str_fsi")
    ### BG Populations
    stn       = Population(params['stn__size'],       izhikevich2003, name="stn")
