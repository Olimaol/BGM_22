...
///////////////////////////////////////////////////////////////
// Main Structure for the population of id 0 (cor_go)
///////////////////////////////////////////////////////////////
struct PopStruct0{
    ...
    // Global parameter tau_up
    double  tau_up ;

    // Global parameter tau_down
    double  tau_down ;
    ...
    double get_global_attribute_double(std::string name) {
        ...
        // Global parameter tau_up
        if ( name.compare("tau_up") == 0 ) {
            return tau_up;
        }

        // Global parameter tau_down
        if ( name.compare("tau_down") == 0 ) {
            return tau_down;
        }
    ...
    void update() {
        ...
        if( _active ) {
            ...
            for(int i = 0; i < size; i++){
                ...
                // dact/dt = if (rates - act) > 0: (rates - act) / tau_up else: (rates - act) / tau_down
                double _act = (-act[i] + rates[i] > 0 ? (-act[i] + rates[i])/tau_up : (-act[i] + rates[i])/tau_down);
    ...
    long int size_in_bytes() {
        ...
        size_in_bytes += sizeof(double);	// tau_up
        size_in_bytes += sizeof(double);	// tau_down
        // Variables
